...
class GifCreator(object):
    ...
    def _get_last_date_fname(self, file_names):
        # lol \o/
        return time.strftime('%d_%b_%Y_%H_%M_%S', sorted(
            [time.strptime(date_str.split('.')[0], "%d_%b_%Y_%H_%M_%S") for
             date_str in file_names], reverse=True)[0])
    ...
    def find_gif_rdy_folders(self):
        while True:
            ...
            for abs_path in full_folder_paths:
                ...
                gif_name = ("giffed_pic_until"
                            "_{last_date}.gif").format(last_date=last_date)
                wildcard = os.path.join(abs_path, "*.jpg")
                subprocess.check_call(['convert', '-delay', '20', '-loop',
                                       '0', wildcard, os.path.join(abs_path,
                                                                   gif_name)])
                used_pic_folder = 'used_pictures'
                processed_pictures_path = os.path.join(
                    abs_path, used_pic_folder)
